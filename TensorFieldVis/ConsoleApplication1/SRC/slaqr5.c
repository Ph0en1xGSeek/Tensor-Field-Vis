...
/*                 ==== A Bulge may collapse because of vigilant   
		    ...
		    if (v[m * v_dim1 + 1] != 0.f && (v[m * v_dim1 + 3] != 0.f 
			    || h__[k + 3 + (k + 1) * h_dim1] == 0.f && h__[k 
			    + 3 + (k + 2) * h_dim1] == 0.f)) {
...
/*                    ==== Atypical case: collapsed.  Attempt to   
			...
			slaqr1_(&c__3, &h__[k + 1 + (k + 1) * h_dim1], ldh, &
				...
				2], &si[m * 2], vt);
			scl = dabs(vt[0]) + dabs(vt[1]) + dabs(vt[2]);
			if (scl != 0.f) {
			    vt[0] /= scl;
			    vt[1] /= scl;
			    vt[2] /= scl;
...
/*                    ==== The following is the traditional and   
                      ...
                      .    test.  ====   
                      . */
			if ((r__1 = h__[k + 1 + k * h_dim1], dabs(r__1)) * (
				dabs(vt[1]) + dabs(vt[2])) > ulp * dabs(vt[0])
				 * ((r__2 = h__[k + k * h_dim1], dabs(r__2)) 
				+ (r__3 = h__[k + 1 + (k + 1) * h_dim1], dabs(
				r__3)) + (r__4 = h__[k + 2 + (k + 2) * h_dim1]
				, dabs(r__4)))) {
...
/*                       ==== Starting a new bulge here would   
			    ...
			    if (v[m * v_dim1 + 2] == 0.f && v[m * v_dim1 + 3] 
				    == 0.f) {
				v[m * v_dim1 + 1] = 0.f;
			    } else {
				h__[k + 1 + k * h_dim1] = beta;
