...
/*  -- LAPACK auxiliary routine (version 3.1) --   
    ...
    extern /* Subroutine */ int sorghr_(integer *, integer *, integer *, real 
	    *, integer *, real *, real *, integer *, integer *), strexc_(char 
...
/*        ==== Workspace query call to SORGHR ==== */
	...
	i__1 = jw - 1;
	sorghr_(&jw, &c__1, &i__1, &t[t_offset], ldt, &work[1], &work[1], &
		c_n1, &info);
	lwk2 = (integer) work[1];
...
/*     ==== Convert to spike-triangular form.  (In case of a   
    ...
    slaset_("A", &jw, &jw, &c_b10, &c_b11, &v[v_offset], ldv);
...
/*           ==== Reflect spike back into lower triangle ==== */
	    ...
	    i__1 = jw - 2;
	    i__2 = jw - 2;
	    slaset_("L", &i__1, &i__2, &c_b10, &c_b10, &t[t_dim1 + 3], ldt);
...
/*        ==== Accumulate orthogonal matrix in order update   
	...
	if (*ns > 1 && s != 0.f) {
	    i__1 = *lwork - jw;
	    sorghr_(&jw, &c__1, ns, &t[t_offset], ldt, &work[1], &work[jw + 1]
		    , &i__1, &info);
	    sgemm_("N", "N", &jw, ns, ns, &c_b11, &v[v_offset], ldv, &t[
		    t_offset], ldt, &c_b10, &wv[wv_offset], ldwv);
	    slacpy_("A", &jw, ns, &wv[wv_offset], ldwv, &v[v_offset], ldv);
	}
...
/* Computing MIN */
	    ...
	    kln = min(i__3,i__4);
	    sgemm_("N", "N", &kln, &jw, &jw, &c_b11, &h__[krow + kwtop * 
...
/* Computing MIN */
		...
		kln = min(i__3,i__4);
		sgemm_("C", "N", &jw, &kln, &jw, &c_b11, &v[v_offset], ldv, &
...
/* Computing MIN */
		...
		kln = min(i__3,i__4);
		sgemm_("N", "N", &kln, &jw, &jw, &c_b11, &z__[krow + kwtop * 
